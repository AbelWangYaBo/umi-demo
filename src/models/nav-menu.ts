...
const genMenu = (pKey: string | null): NavMenuModelState[] => {
  const navs = [
    {
      "label": "Home",
      "key": "0"
...
interface NavMenuModelState {
  key: string;
  label: string;
  children?: NavMenuModelState[];
}

interface NavMenuModelType {
  namespace: 'navMenu';
  effects: {
    query: Effect;
  };
  state: {
    menus: NavMenuModelState[]
  };
  reducers: {
    save: Reducer<NavMenuModelState>;
    // 启用 immer 之后
...
const NavMenuModelStore: NavMenuModelType = {
  namespace: 'navMenu',
  ...
  subscriptions: {
    setup({ dispatch }) {
      console.log('NavMenuModelStore subscriptions')
      dispatch({
        type: 'query'
      })
    }
...
export default NavMenuModelStore;
